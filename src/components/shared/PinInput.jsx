    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
      <div className="bg-[#1E293B] rounded-2xl p-6 w-80 border border-[#334155]">
        ...
        <div className="grid grid-cols-3 gap-3">
          {keys.flat().map((k, i) => {
            if (k === 'C') return (
              <button key={i} onClick={handleClear} className="bg-[#EF4444] text-white rounded-xl py-4 text-xl font-bold active:scale-95 transition">C</button>
            )
            if (!k) return <div key={i} />
            return (
              <button key={i} onClick={() => handleDigit(k)} className="bg-[#334155] text-white rounded-xl py-4 text-xl font-bold active:scale-95 hover:bg-[#475569] transition">{k}</button>
            )
          })}
        </div>
      </div>
